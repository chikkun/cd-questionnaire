...
(function($){
    ...
    jQuery.fn.sheepIt = function (options){
        ...
        // changed auby chiku
        function getOrSetTemplate(element, attrname, flag, natt){
	    if(!flag) {
		...
		if(norm && !natt) {
		    ...
		    var att=element.attr(attrname);
		    // after normalizing, save id or name 
		    if(natt) {
		...
		getOrSetTemplate(that, "name", true, newNameAttr); // changed by chiku
                that.attr("name", newNameAttr);

                /* Normalize field id attributes */
                newIdAttr = idTemplateAttr.replace(options.indexFormat, index);
		getOrSetTemplate(that, "id", true, newIdAttr); // changed by chiku
		...
		//  changed '#index#' to options.indexFormat by chiku
...
//                }
	    ...
	    if(type == undefined){ // added by chiku
		type = field[0].nodeName.toLowerCase();
            }
        ...
        function initialize()
        {
            ...
            if ( options.iniFormsCount > getFormsCount()) {
                ...
                for (x=1; x<=b; x++) {
                    addForm(true);// change false to true by chiku
                }
