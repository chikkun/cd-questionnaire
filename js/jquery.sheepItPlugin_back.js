...
(function($){
    ...
    jQuery.fn.sheepIt = function (options){
        ...
        function getOrSetTemplate(element, attrname, flag, natt){ //chiku
	    if(!flag) { //chiku
		...
		var norm=element.attr(attrname+"normalized");
		if(norm && !natt) {
		    return unescape(norm);
		} else{
		    var att=element.attr(attrname);
		    if(natt) {
			element.attr(attrname+"normalized", escape(natt));
		    } else {
			element.attr(attrname+"normalized", escape(att));
		    }
		}
		return att;
	    }
        ...
        function normalizeFieldsForForm(form, index)
        {
            form.find(formFields).each(function(){
                var that = $(this)
                    ,idTemplateAttr = getOrSetTemplate(that,"id", true)
                    ,nameTemplateAttr = getOrSetTemplate(that, "name", true)
                    ,idAttr = that.attr("id")
                    ,nameAttr = that.attr("name")
                    
                /* Normalize field name attributes */
                newNameAttr = nameTemplateAttr.replace(options.indexFormat, index);
		getOrSetTemplate(that, "name", true, newNameAttr); // chiku
                that.attr("name", newNameAttr);
		if (options.parentFlag == false){// chiku
		    //console.log(options.parentIndexFormat);
		}
                /* Normalize field id attributes */
                newIdAttr = idTemplateAttr.replace(options.indexFormat, index);
		getOrSetTemplate(that, "id", true, newIdAttr); // chiku
		...
		if(form.data("parentFlag") == true) {// chikkun
		    form.data("parentIndex", index);
		} else {
		    form.data("childIndex", index);
		}
            }
...
//                if (index.indexOf('#form#') != -1 || index.indexOf('#index#') != -1) {
                    ...
                    index = index.replace(options.indexFormat, formIndex);
		    console.log("00000000" + index + "//" + options.indexFormat);
//                } else {
